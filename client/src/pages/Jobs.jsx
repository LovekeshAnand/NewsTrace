...
export default function Jobs() {
  ...
  usePolling(fetchScrapeJobs, 5000, []);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="px-4 py-2 bg-slate-800 border border-slate-700 text-white rounded-lg hover:bg-slate-700 transition-all flex items-center gap-2"
        >
          <RefreshCw size={18} />
          Refresh
        </button>
      ...
      {scrapeJobs.length === 0 ? (
        <div className="text-center py-20 bg-slate-800 rounded-xl border border-slate-700">
          ...
          <p className="text-sm text-slate-500 mt-2">Start one from the dashboard!</p>
        </div>
      ) : (
...
function JobCard({ job, getStatusColor }) {
  return (
